...
def checkSimilarity(window, path1, path2):
    ...
    if(result <= THRESHOLD):
       messagebox.showerror("Failure: Fingerprints Do Not Match",
                             "Fingerprints are "+str(result)+f" % similar!!")
       pass
    else:
       messagebox.showinfo("Success: Fingerprints Match",
                            "Fingerprints are "+str(result)+f" % similar!!")
    return True


root = tk.Tk()
root.title("Fingerprint Matching Project")
root.geometry("500x700") 
x_label = tk.Label(root, text="Compare Two Fingerprints:",bg='yellow',fg='red',font=("Comicsans,15,bold"),borderwidth=15)
x_label.pack()

img1_message2 = tk.Label(root, text="Fingerprint 1", font=10)
...
img2_message = tk.Label(root, text="Fingerprint 2", font=10)
